...
int main() {
    ...
    fout << "3x3 Matrix Determinant Analysis (Scalars 1–40)\n";
    fout << "==============================================\n\n";
    ...
    while (getline(fin, line)) {
        if (line.find("Matrix #") != string::npos) {
            ...
            if (!matrix.empty()) {
                ...
                // Determinant scaling property: det(kA) = k^3 * det(A)
                for (int s = 1; s <= 40; s++) {
    ...
    // Process the last matrix if not followed by another
    if (!matrix.empty()) {
        matrixCount++;
        fout << "Matrix #" << matrixCount << " (3x3)\n";
        fout << "----------------------------------------------\n";
