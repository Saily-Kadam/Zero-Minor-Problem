...
// Function to calculate determinant of 3x3 matrix directly
...
int main() {
    ...
    fout << "3x3 Matrix Determinant Analysis (Scalars 1–36)\n";
    fout << "=============================================\n\n";
    ...
    while (getline(fin, line)) {
        if (line.find("Matrix #") != string::npos) {
            if (!matrix.empty()) {
                ...
                // Determinants for scalars 1 to 36
                vector<double> dets;
                for (int s = 1; s <= 36; s++) {
                    double det = pow(s, 3) * determinant3x3(matrix);
                    dets.push_back(det);
    ...
    // Process last matrix if not followed by a new one
    if (!matrix.empty()) {
        matrixCount++;
        fout << "Matrix #" << matrixCount << " (3x3)\n";
        fout << "---------------------------------------------\n";
